...
class Master():
    ...
    def __init__(self, num_ids_per_assignment, file_name, Master_type): #INHERITED CLASSES WILL SET Master_TYPE
        ...
        print("Master Initiated & Awaiting Input Data Request")
...
#PREPARATION METHODS
    ...
    def prepare_scope(self):
        ...
        if self.is_csv(): #IF WE HAVE A CSV, COMPARE REQUESTED DATA TO CSV DATA
            ...
            for id in self.work_queue: #IDS NOT IN CSV DATA ADDED TO TO_BE_SCRAPED LIST
            ...
            self.ids_to_be_scraped = self.work_queue
        ...
        num_work_queue = len(self.work_queue)
        requested_ids_found = num_work_queue - self.num_ids_to_be_scraped

        print("{found} of {requested} Requested {type} Found In Log. {outstanding} {type} Remaining".format(found = requested_ids_found, requested = num_work_queue, type = self.data_type_name, outstanding = self.num_ids_to_be_scraped))

    def generate_assignments(self): #SPLITS DATA NEEDED INTO ASSIGNMENTS THAT CAN BE GIVEN TO SlaveS
    ...
    def prepare_for_Slaves(self):

        if not self.work_queue:
            print("Master Requires Data Request Prior to Preparing for Slaves")
            return
        ...
        print("Master is Ready to Recieve Requests from Slaves")
...
#COMMUNICATE WITH SlaveS METHODS
    ...
    def give_assignment(self):
        ...
        else: #IF THERE ARE NO OUTSTANDING ASSIGNMENTS, SELECT NONE (NONE TRIGGERS A SHUTDOWN FOR Slave)
            assignment_key = None
        ...
        #GIVE KEY AND ASSIGNMENT ID TO Slave
...
#LOG DATA METHODS
    ...
    def remove_assignment_from_list_of_outstanding(self, assignment_key):
        ...
        except ValueError: #AT THE END OF THE PROCESS, MULTIPLE SlaveS MAY HAVE BEEN GIVEN THE SAME ASSIGNMENT
            pass
...
class Review_Master(Master):
    ...
    def input_data_request(self, min_id, max_id):
        self.work_queue = range(min_id, max_id)

        print("Master Recieved Data Request & Ready to Prepare for Slaves")
...
#Slave WITH REVIEW_Slave AND BOOK_Slave INHERITING
#Slave HANDLES:
    #SCRAPING & PARSING
...
class Slave():
    ...
    def __init__(self, Master, max_sleep_time, Slave_type): #INHERITED CLASSES WILL SET Slave TYPE
    ...
    def request_assignment(self):
        self.assignment_key, self.assignment_ids = self.Master.give_assignment()
    ...
    def log_data(self): #Slave KEEPS DATA AS A LIST OF NODES
    ...
    def transmit_data_to_Master(self):

        self.Master.input_data(self.assignment_key, self.collected_data_nodes_list)
        self.completed_queue= [] #ONCE DATA IS TRANSMITTED, CLEAR OUT LIST
...
class Review_Slave(Slave):
    ...
    def __init__(self, Master, max_sleep_time):
        super().__init__(Master, max_sleep_time, "review")
...
class Book_Slave(Slave):
    ...
    def __init__(self, Master, max_sleep_time):
        super().__init__(Master, max_sleep_time, "book")
